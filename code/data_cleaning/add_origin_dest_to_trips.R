...
files_data_tix
files_data_tix <- files_data_tix[ str_detect( files_data_tix, "venda_passagem_\\d{2}_\\d{4}.csv" ) ]
...
list_data_tix <- lapply( files_data_tix,
                         fread,
                         select = c( "nu_linha", "origem_destino_linha" ) )
str( list_data_tix, 1 ) 
...
data_trips <- merge.data.table( x = data_trips, y = data_tix, by = "nu_linha", all.x = TRUE )
